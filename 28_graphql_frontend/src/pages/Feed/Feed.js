...
class Feed extends Component {
  ...
  finishEditHandler = postData => {
    ...
    let urlImage = `${
      navigator.userAgent.indexOf('Win') !== -1
        ? 'http://localhost:8080/put-image'
        : 'http://192.168.1.30:8080/put-image'
    }`;
    ...
    fetch(urlImage, {
      ...
      .then(res => res.json())
      .then(fileResData => {
        const imageUrl = fileResData.filePath || 'dummyUrl';
        ...
        let graphqlQuery = this.state.editPost
          ? {
      ...
      .then(resData => {
        ...
        if (
          ...
          throw new Error(
            `${this.state.editPost ? 'Updating' : 'Creating new'} post failed!`
          );
        }
        console.log(resData);
