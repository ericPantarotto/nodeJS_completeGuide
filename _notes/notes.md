...
## Implementing a Token Logic

With a **unique token**  generated by our app, we can verify that the user did get that link from us because if we just, well let the user now change that password.

**<span style='color: #bcdbf9'> Note:**  *node.js* has a built-in crypto library which I can use for that, This is a library that helps us with creating secure unique random values and other things.

## Why Editing Fails

*Mongoose* `req.user._id` returns an `ObjectId` while `req.user.id` directly returns the string.  
it means that `req.user.id` === `req.user._id.toString()`
...
Here's one important note about hosting our app on Heroku/Render!

The user-generated/ uploaded images, are saved and served as intended. But like all hosting providers that offer virtual servers, your file storage is not persistent! Your source code is saved and re-deployed when you shut down the server (or when it goes to sleep, as it does automatically after some time in the Heroku/Render free tier).

But your generated and uploaded files are not stored and re-created. They would be lost after a server restart! Therefore, it's recommended that you use a different storage place when using such a hosting provider. In cases where you run your own server, which you fully own/ manage, that does of course not apply.
...
**<span style='color: #bcdbf9'> Note:** Deno will compile the code from TS to JS and the V8 Javascript engine wrapped by Deno will execute that JS code

## Deno Runtime

When you're working with Node, you have these core Node modules, for example, a module for working with the file system. and with Node, you simply import such a module, and then you can start reading or writing files for example.  
You also have a `Promise based API` version available for some of these core modules.

Now for Deno, we have a similar concept. There we have this so-called runtime API here, also sometimes called the namespace APIs.

These are core APIs, core features, which are built into Deno, and which are available on a Deno object, which in turn is globally available in any Deno code you're executing.

Here is a difference compared to Node already.  
In Node, You also have some core APIs available, some core features available, either after importing them from the respective core module, or things like set timeout also are globally available in Node scripts.

But things like the fetch API are not available in Node.

Because Deno has the core philosophy of being as browser compatible as possible. That means that any code that could run outside of a browser is also supported in Deno.

Something like the fetch API for sending HTTP requests. This is a available in Deno because this is not strictly tied to a browser.

because Deno runs out sort of a browser, but which wouldn't exist inside of a browser. And there are features which we can use in a browser, for example, features to interact with the browser Dom, which are not supported in Deno (and also not supported in Node of course), because there we have no Dom there we are not running inside of a browser.
